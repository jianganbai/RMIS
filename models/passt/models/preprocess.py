...
class AugmentMelSTFT(nn.Module):
    def __init__(self, n_mels=128, sr=32000, win_length=800, hopsize=320, n_fft=1024, freqm=48, timem=192,
        ...
        self.register_buffer(
            'eval_mel_basis',
            self._build_mel_basis(self.fmin, self.fmax),
            persistent=False,
        )
        if freqm == 0:
    ...
    def _build_mel_basis(self, fmin, fmax):
        ...
        return torch.nn.functional.pad(mel_basis, (0, 1), mode='constant', value=0)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            mel_basis = self._build_mel_basis(fmin, fmax).to(device=x.device, dtype=x.dtype)
        else:
            mel_basis = self.eval_mel_basis
            if mel_basis.device != x.device or mel_basis.dtype != x.dtype:
                mel_basis = mel_basis.to(device=x.device, dtype=x.dtype)
