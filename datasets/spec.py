...
class base_stft:
    def __init__(
        ...
        self.freq_first = stft_conf.get('freq_first', False)
    ...
    def cal_valid_len(
        ...
        time_idx = -1 if self.freq_first else -2
        valid_len = min((valid_len - self.n_fft) // self.hop_length + 1, x.shape[time_idx])
        return valid_len
...
class torchaudio_stft(base_stft):
    ...
    def __call__(
        ...
        x = x.squeeze()
        if not self.freq_first:
            x = x.transpose(-2, -1)  # [time, freq]
...
class base_fbank:
    def __init__(
        ...
        self.freq_first = fbank_conf.get('freq_first', False)
